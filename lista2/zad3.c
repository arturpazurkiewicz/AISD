        ...
        if (type != 0) {
            ...
            for (int i = 100; i < 10001; i += 100) {
                ...
                for (int j = 0; j < k; ++j) {
                    ...
                    for (int l = 0; l < i; ++l) {
                        l1[l] = l2[l] = l3[l] = l4[l] = pcg32_random_r(&rng);
                    }
                    t1 += sorter(l1, &c1, &m1, comp, 1, i);
                    t2 += sorter(l1, &c2, &m2, comp, 2, i);
                    t3 += sorter(l1, &c3, &m3, comp, 3, i);
                    t4 += sorter(l1, &c4, &m4, comp, 4, i);

                }
                fprintf(file, " %d;%d; %f; %f; %lf;%f; %f; %lf;%f; %f; %lf;%f; %f; %lf\n", i, k,
            ...
            double time = 0;
            time = sorter(list, &compares, &moves, comp, type, n);
            for (int i = 0; i < n; i++)
                printf("%d ", list[i]);
            printf("Size: %d\n", n);
        }
